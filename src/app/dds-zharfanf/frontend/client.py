...
class Client(AppServer):
    ...
    def __init__(self, hname, config, server_handle=None,
        ...
        super().__init__(uri, edge_uri, control_port, data_port)
    ...
    def change_bandwidth_and_adapt(self, resource_change: float):
    ...
    def prep_profiling(self) -> Any:
        ...
        finishInfer = datetime.now()
        appNum = os.popen('pwd').read()
        appNum = int(appNum[appNum.rfind("app")+3])
        f = open("../../debugTimeFinishInfer-%d.txt" %(appNum), "a")
        f.write("finish inferring at "+ str(finishInfer)+ '\n') # di sini mungkin udah beda, ada yang cepet, ada juga yang lambat
        f.close()
    ...
    def get_diff(self) -> InferDiff:
        ...
        if self.config.baseline_mode:
            return InferDiff(infer_diff=0, latency_diff=0, infer_diff_high=0, infer_diff_low=0, bw_lower_bound=0)
    ...
    def change_to_best_configuration(self):
        try:
            low_res_best, low_qp_best, high_res_best, high_qp_best, f1_best = get_best_configuration(self.current_bw, f'{self.config.profile_folder_path}/{self.config.profile_folder_name}/profile-{self.profile_no}.csv')
    ...
    def analyze_video(
        ...
        for i in range(0, nframes, self.config.batch_size):
            ...
            if self.just_reallocated.is_set():
                ...
                f = open("../../debugTime-%d.txt" %(appNum), "a")
                f.write("finish realloc at "+ str(finishRealloc)+ '\n') # keknya gak harus selalu di sini sih dan gak bareng juga kayaknya
                f.close()
            ...
            transmissionDelay += perf_counter_s() - transmissionStart
        ...
        f = open("../../e2eDelay-%d.csv" %(appNum), "a")
        f.write(str(e2eTime) + '\n')
        f.close()

        f = open("../../encodingDelay-%d.csv" %(appNum), "a")
        ...
        f = open("../../transmissionDelay-%d.csv" %(appNum), "a")
        f.write(str(transmissionDelay) + '\n')
        f.close()

        f = open("../../optimizeAndReallocDelay-%d.csv" %(appNum), "a")
        f.write(str(optimizeAndReallocDelay) + '\n')
        f.close()

        f = open("../../tempTime-%d.csv" %(appNum), "a")
