...
class Detector:
    ...
    def __init__(self, model_path='frozen_inference_graph.pb'):
        ...
        # config = ConfigProto(intra_op_parallelism_threads=5, inter_op_parallelism_threads=25, allow_soft_placement=True)
        config = ConfigProto()
    ...
    def run_inference_for_single_image(self, image, graph):
        with self.d_graph.as_default():
            ...
            # TODO: AWStream doesn't need rpn intermedia results
    ...
    def run_inference_for_multiple_images(self, images, graph):
        with self.d_graph.as_default():
            ...
            infer_end = perf_counter_s() - infer_start
            writeResult(appNum, infer_end, "inferDebug")
            # FOR RPN intermedia results
            # w = output_dict[key_tensor_map['Resized_shape']][1]
            # h = output_dict[key_tensor_map['Resized_shape']][0]
            # input_shape_array = np.array([h, w, h, w])
