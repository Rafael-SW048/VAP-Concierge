...
class Server:
    ...
    def perform_detection(self, images_direc, resolution, fnames=None,
        ...
        # TODO: Implement tensor processing
        # Tensor processing
        if fnames is None:
        ...
        detection_results, rpn_results, infer_temp = (
                self.detector.infer(imagesTemp))
        infer_delay = infer_temp
        for i in range(imagesTemp.shape[0]):
            ...
            self.logger.debug(
                ...
                f"and {len(rpn_regions)} for {fnames[i]}")
    ...
    #                 Region(fid, 0, 0, 0, 0, 0.1, "no obj", resolution))
        ...
        return final_results, rpn_regions, infer_delay
    ...
    def emulate_high_query(self, vid_name, low_images_direc, req_regions):
        ...
        return results_with_detections_only, infer_delay
    ...
    def perform_low_query(self, vid_data, start_fid=None):
        ...
        # Perform server side cleanup for the next batch, aws only performs one iteration
    ...
    def perform_high_query(self, file_data):
        ...
        results, infer_delay = self.emulate_high_query(
