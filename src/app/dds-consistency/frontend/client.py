...
class Client(AppServer):
    ...
    def get_diff(self) -> InferDiff:
        ...
        # Bandwidth reduction
        # Calculate the infer_diff_high first
        if self.final_results.regions_dict == self.segmental_ground_truth.regions_dict:
            ...
            if self.infer_diff_high == -1:
                self.infer_diff_high = 1
        # self.infer_diffs.append(1-self.infer_diff_high)
        # self.acc_sens.append(self.f1-self.real_f1)
        # self.f1s.append(self.real_f1)
        # self.logger.info("Reading ground truth results complete")
        # if self.min_fid == 0:
        #     writeResult(self.app_idx, f"real: {self.real_low_resolution}, {self.real_high_resolution}, {self.real_low_qp}, {self.real_high_qp}, current: {self.config.low_resolution}, {self.config.high_resolution}, {self.config.low_qp}, {self.config.high_qp}", "configDebug")
        #     self.segmental_ground_truth.write_results_csv(f"debugAppGT{self.app_idx}-high.csv")
        #     self.final_results.write_results_csv(f"debugApp{self.app_idx}-high.csv")
        ...
        # if self.min_fid == 0:
        #     # writeResult(self.app_idx, f"real: {self.real_low_resolution}, {self.real_high_resolution}, {self.real_low_qp}, {self.real_high_qp}, current: {self.config.low_resolution}, {self.config.high_resolution}, {self.config.low_qp}, {self.config.high_qp}", "configDebug")
        #     self.segmental_ground_truth.write_results_csv(f"debugAppGT{self.app_idx}-low.csv")
        #     self.final_results.write_results_csv(f"debugApp{self.app_idx}-low.csv")
        ...
        # Infer_diff Implementation
        # total_infer_diff = 2 - (self.infer_diff_high + self.infer_diff_low)
        # if total_infer_diff == 0:
        #     total_infer_diff = 1


        return InferDiff(
            infer_diff = 0,
            infer_diff_high = (1-self.infer_diff_high),
            infer_diff_low = (1-self.infer_diff_low),
            latency_diff = 0,
    ...
    def analyze_video(
        ...
        for i in range(0, nframes, self.config.batch_size):
            ...
            self.change_bandwidth_and_adapt(self.profiling_delta)
            if self.final_results.regions_dict == self.segmental_ground_truth.regions_dict:
                self.infer_diff_high = 1
            else:
                _, _, _, _, _, _, self.infer_diff_high = evaluate_partial(self.min_fid,
                ...
                if self.infer_diff_high == -1:
                    self.infer_diff_high = 1
            self.acc_sen_high = self.f1-self.real_f1
            self.change_bandwidth_and_adapt(-2*self.profiling_delta)
            ...
            if self.final_results.regions_dict == self.segmental_ground_truth.regions_dict:
                ...
                if self.infer_diff_low == -1:
                    self.infer_diff_low = 1
            self.acc_sen_low = self.real_f1 - self.f1
            # Bandwidth correction
            self.change_bandwidth_and_adapt(2*self.profiling_delta, True)
            self.infer_diffs.append((1-self.infer_diff_high, 1-self.infer_diff_low))
            self.acc_sens.append((self.acc_sen_high, self.acc_sen_low))
