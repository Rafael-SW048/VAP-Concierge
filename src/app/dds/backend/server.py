...
from app.dds.backend.object_detector import Detector
...
class Server():
    ...
    def __init__(self, config, nframes=None):
        ...
        self.curr_fid = 0
        self.curr_profiling_fid = 0
        self.nframes = nframes
        self.last_requested_regions = None
        self.last_profiling_requested_regions = None
    ...
    def reset_state(self, nframes):
        ...
        for f in os.listdir("server_profiling_temp"):
            os.remove(os.path.join("server_profiling_temp", f))
        for f in os.listdir("server_profiling_temp-cropped"):
            os.remove(os.path.join("server_profiling_temp-cropped", f))

    def perform_server_cleanup(self, is_profiling):
        if not is_profiling:
            for f in os.listdir("server_temp"):
                os.remove(os.path.join("server_temp", f))
            for f in os.listdir("server_temp-cropped"):
    ...
    def perform_low_query(self, vid_data, is_profiling):
        # Write video to file
        dir_name = "server_profiling_temp" if is_profiling else "server_temp"
        with open(os.path.join(dir_name, "temp.mp4"), "wb") as f:
            f.write(vid_data.read())

        # Extract images
        # Make req regions for extraction
        start_fid = self.curr_profiling_fid\
            if is_profiling else self.curr_fid
        ...
        extract_images_from_video(dir_name, req_regions)
        fnames = [f for f in os.listdir(dir_name) if "png" in f]

        results, rpn = self.perform_detection(
            dir_name, self.config.low_resolution, fnames)
        ...
        detections, regions_to_query = self.simulate_low_query(
            start_fid, end_fid, dir_name, batch_results.regions_dict,
        ...
        if not is_profiling:
            ...
            self.last_profiling_requested_regions = regions_to_query
            self.curr_profiling_fid = end_fid
    ...
    def perform_high_query(self, file_data, is_profiling):
        low_images_direc = "server_profiling_temp"\
            if is_profiling else "server_temp"
        cropped_images_direc = "server_profiling_temp-cropped"\
            if is_profiling else "server_temp-cropped"

        with open(os.path.join(cropped_images_direc, "temp.mp4"), "wb") as f:
            f.write(file_data.read())

        regions = self.last_profiling_requested_regions\
            if is_profiling else self.last_requested_regions
        results = self.emulate_high_query(
            low_images_direc, low_images_direc, regions)
        ...
        self.perform_server_cleanup(is_profiling)
