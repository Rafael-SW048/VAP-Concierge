...
class Results:
    def __init__(self):
        ...
        self.regions_dict: Dict[int, List[Region]] = {}
...
def crop_images(results, vid_name, images_direc, resolution=None):
    ...
    for idx, (_, frame) in enumerate(frames):
        if resolution:
            ...
            frame = im_to_write
        filename = os.path.join(vid_name, f"{str(idx).zfill(10)}.png")
...
def filter_results(bboxes, gt_flag, gt_confid_thresh, mpeg_confid_thresh,
    ...
    for b in bboxes:
        ...
        if (confid >= confid_thresh and w*h <= max_area_thresh):
            result.append(b)
    return result
...
def evaluate(max_fid, map_dd, map_gt, gt_confid_thresh, mpeg_confid_thresh,
             max_area_thresh_gt, max_area_thresh_mpeg, iou_thresh=0.5):
    tp_list = []
    fp_list = []
    fn_list = []
    count_list = []
