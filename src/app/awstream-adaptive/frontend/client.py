...
class Client(AppServer):
    ...
    def __init__(self, hname, config, server_handle=None,
        ...
        self.current_budget = 0.35 #default value
    ...
    def default_callback(self, is_succeed: bool, resource_change: float, resource_t: ResourceType) -> bool:
        if (resource_t == ResourceType.BW):
            ...
            if self.is_profiling.is_set():
                self.change_bandwidth_and_adapt(resource_change*0.35)
            else:
    ...
    def get_diff(self) -> InferDiff:
        ...
        if self.final_results.regions_dict == self.segmental_ground_truth.regions_dict:
            ...
            _, _, _, _, _, _, self.infer_diff_high = evaluate_partial(self.min_fid,
                ...
                self.config.low_threshold, 0.4, 0.4, 0.4)
        ...
        # Reduce Bandwidth
        self.change_bandwidth_and_adapt(-2*self.resource_change*0.35)
        ...
        self.change_bandwidth_and_adapt(2*self.resource_change*0.35, True)
        ...
        return InferDiff(
            ...
            # infer_diff_high = (acc_sen_high),
            # infer_diff_low = (acc_sen_low),
            latency_diff = 0,
    ...
    def wait_profiling_done(self):
        ...
        while self.is_profiling.is_set():
            ...
            sleep(0.01)
        # Start encoding for the next segment
    ...
    def analyze_video(
        ...
        # The first segment is already encoded
        # Encode segment_{n+1} and process segment_{n}
        for i in range(start_fid, nframes, self.config.batch_size):
            ...
            # Worker starts in the background, once self.is_profiling is set True, encode video
            ...
            self.logger.info(f"Processing frames {start_frame} to {self.end_frame}, is_profiling: {self.is_profiling.is_set()}")
            ...
            # total_delay = [backendTime, inferTempFirst, backendTime-decode_temp-inferTempFirst, decode_temp]
            ...
            # writeResult(self.app_idx, os.path.isdir(vid_name + "-base-phase-cropped"), "sanityDir")
            ...
            self.logger.info(f"done inferrring......, is_profiling: {self.is_profiling.is_set()}, is_inferring: {self.is_inferring.is_set()}")
            # VAP-Concierge Collaboration End
    ...
    def log_bandwidth_limit(self, frame_id, bandwidth_limit, just_reallocated):
        ...
        self.logger.info(f"Logging bandwidth info, is_profiling: {self.is_profiling.is_set()}")
    ...
    def encode_video_segment(self, vid_name, start_frame, raw_images, enforce_iframes, nframes, is_next=True):
        ...
        low_res_best, low_qp_best, high_res_best, high_qp_best, f1_best, is_min_bw = get_best_configuration(self.budget_trans, 
                                f'{self.config.profile_folder_path}/{self.config.profile_folder_name}/'
                                f'profile-{profile_no}.csv')
        # encode video
        req_regions = Results()
