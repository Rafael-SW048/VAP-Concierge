...
class Results:
    ...
    def add_multiple_results(self, regions_to_add: Region, fid, threshold=0.5):

        # Add anyway if threashold = 1
        not_dup = []
        if threshold == 1:
            self.append_batch(regions_to_add, fid)
            return
        for region in regions_to_add:
            region.origin = "low-res"
            dup_region = self.is_dup(region, threshold) 
            if (not dup_region or # If no duplicate exists, OR
                    ("tracking" in region.origin and
                    "tracking" in dup_region.origin)): # The origin is tracking for both regions
                not_dup.append(region)
            
        # Add the region to regions and regions_dict
        self.regions.append_batch(not_dup)
        if fid not in self.regions_dict:
            self.regions_dict[fid] = []
        self.regions_dict[fid].append_batch(not_dup)
    ...
    def append_batch(self, regions_to_add, fid):
        self.regions += regions_to_add
        if fid not in self.regions_dict:
            self.regions_dict[fid] = []
        self.regions_dict[fid] += regions_to_add
...
def extract_images_from_video(images_path, req_regions, executor):
    # Instead of decoding through ffmpeg, we may decode the frames using opencv
    ...
    paths = []
    frames = []
    cap = cv.VideoCapture(encoded_vid_path)
    idx = 0
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret:
            # thread = Thread(target=cv.imwrite, args=(os.path.join(images_path, f"{str(idx).zfill(10)}.png"), frame, [cv.IMWRITE_PNG_COMPRESSION, 0],))
            paths.append(os.path.join(images_path, f"{str(idx).zfill(10)}.png"))
            frames.append(frame)
            # threads.append(thread)
        else:
            break
        idx += 1
    processes = [executor.submit(cv.imwrite, paths[i], frames[i], [cv.IMWRITE_PNG_COMPRESSION, 0]) for i in range(len(frames))]
    ...
    # decoding_result = subprocess.run(["ffmpeg", "-y", "-hwaccel", "cuda",
    #                                   "-i", encoded_vid_path,
    #                                   "-pix_fmt", "yuvj420p",
    #                                   "-g", "8", "-q:v", "2",
    #                                   "-vsync", "0", "-start_number", "0",
...
def merge_images_sub(cropped_images_direc, low_images_direc, req_regions, fid, fname):
    # thread_func = concurrent.futures.ThreadPoolExecutor(max_workers=2)
    # # Read high resolution image
    high_image = cv.imread(os.path.join(cropped_images_direc, fname))
    low_image = cv.imread(os.path.join(low_images_direc, fname))
    # results = [thread_func.submit(cv.imread, os.path.join(cropped_images_direc, fname)), thread_func.submit(cv.imread, os.path.join(low_images_direc, fname))]

    # # Read low resolution image
    # high_image = results[0].result()
    # low_image = results[1].result()
    width = high_image.shape[1]
    height = high_image.shape[0]
    # Enlarge low resolution image
    enlarged_image = cv.resize(low_image, (width, height), fx=0, fy=0,
...
def merge_images_multi(cropped_images_direc, low_images_direc, req_regions, p):
    images = {}
    fids = []
    fnames = []
    # req_regions_list = [req_regions for i in range(5)]
    req_regions_list = []
    low_images_direc_list = [low_images_direc for i in range(5)]
    cropped_images_direc_list = [cropped_images_direc for i in range(5)]
    for fname in os.listdir(cropped_images_direc):
        if "png" not in fname:
            continue
        fid = int(fname.split(".")[0])
        fids.append(fid)
        fnames.append(fname)
        req_regions_list.append(req_regions.regions_dict[fid])
    # results = [[thread_func.submit(cv.imread, os.path.join(cropped_images_direc, fname)), thread_func.submit(cv.imread, os.path.join(low_images_direc, fname))] for fname in fnames]
    # high_images = [f[0].result() for f in results]
    # low_images = [f[1].result() for f in results]
    zippedArgs = list(zip(cropped_images_direc_list, low_images_direc_list, req_regions_list, fids, fnames))
    processes = p.starmap(merge_images_sub, zippedArgs)
    for i in range(len(processes)):
        images[fids[i]] = processes[i]
...
def cleanup(vid_name, debug_mode=False, start_id=None, end_id=None):
    ...
    if not debug_mode:
        shutil.rmtree(vid_name + "-base-phase-cropped", ignore_errors=True)
        shutil.rmtree(vid_name + "-cropped", ignore_errors=True)
    else:
...
def get_best_configuration(bandwidth_limit, profile_path) -> Tuple[float, float, float, float, float, bool]:
    ...
    is_min_bw = False
    best_profile = {"low-resolution":None, "low_qp": None, "high-resolution": None, "high_qp": None,
                    "F1": None}
    try:
...
def get_regions_to_query_multi(rpn_regions, detections, objfilter_iou, rpn_enlarge_ratio, intersection_threshold, size_obj): 
    req_regions = Results()
    for region in rpn_regions:
        ...
        req_regions.add_single_result(
            region, intersection_threshold)
    return req_regions

def combine_regions(fid, detection_results, rpn_results, min_object_size, resolution):
    temp_results = []
    temp_rpn_regions = []
    ...
    #     f"and {len(temp_rpn_regions)} for {fname}")
...
def first_phase_sub(output_dict, num, detector):
    results = []
    results_rpn = []
    result_temp = []
    for i in range(len(output_dict['detection_boxes'][num])):
        object_class = output_dict['detection_classes'][num][i]
        ...
        ymin, xmin, ymax, xmax = output_dict['detection_boxes'][num][i]
        confidence = output_dict['detection_scores'][num][i]
        box_tuple = (xmin, ymin, xmax - xmin, ymax - ymin)
        result_temp.append((object_class, confidence, box_tuple))
    results = result_temp

    result_temp = []
    # for idx_region, region in enumerate(output_dict['RPN_box_normalized'][num]):
    #     x = region[1]
    #     y = region[0]
    #     w = region[3] - region[1]
    #     h = region[2] - region[0]
    #     conf = output_dict['RPN_score'][num][idx_region]
    #     if conf < detector.rpn_threshold or w * h == 0.0 or w * h > 0.04:
    #         continue
    #     result_temp.append(("object", conf, (x, y, w, h)))
    results_rpn = result_temp
    return [results, results_rpn]

def second_phase_sub(output_dict, num, detector):
    results = []
    results_rpn = []
    result_temp = []
    for i in range(len(output_dict['detection_boxes'][num])):
