...
class DDS(AppServer):
    ...
    def get_diff(self) -> InferDiff:
        ...
        inference_up = read_cache_result(
        ...
        if DEBUG:
            ...
            # get acc_up
            max_f1_up = calculate_diff_concurrent(
                    inference_up, gt_dict, self.iou_threshold,
                    pool=process_pool).infer_diff
            ...
            # get acc_down
            _, max_f1_smpl_p = self._adapt(max_bw_down)
            inference_down = read_cache_result(
                self.all_results, self.app_idx, start_fid, end_fid,
                *max_f1_smpl_p.config.pack()
                )
            max_f1_down = calculate_diff_concurrent(
                    inference_down, gt_dict, self.iou_threshold,
                    pool=process_pool).infer_diff
            ...
            infer_diff_up: float = calculate_diff_concurrent(
                self.prev_inference, inference_up, self.iou_threshold,
                pool=process_pool).infer_diff
            self.logger.debug(
    ...
    def _read_bw_profile(self) -> List[SamplePoint]:
        ...
        # with open("/home/cc/vap-concierge/src/app/dds/dds_profile","r")\
        with open(os.path.join(self.data_src, "pf"), "r")\
                as bw_profile_fd:
    ...
    def _adapt(self, max_bw: Optional[float] = None)\
            -> Tuple[bool, SamplePoint]:
        if max_bw is None:
            max_bw = self.current_bw
        max_f1_smpl_p: SamplePoint = self.profile_curve[0]
        for smpl_p in self.profile_curve:
            if (smpl_p.f1 > max_f1_smpl_p.f1
                    and smpl_p.bw_used <= max_bw):
                max_f1_smpl_p = smpl_p
        will_backlog: bool =\
            self._read_next_batch_bw(max_f1_smpl_p.config) > max_bw
